...
namespace charutils
{
    ...
    void TrySkillUP(CCharEntity* PChar, SKILLTYPE SkillID, uint8 lvl, bool forceSkillUp, bool useSubSkill)
    {
        ...
        if (((PChar->WorkingSkills.rank[rawSkillID] != 0) && !(PChar->WorkingSkills.skill[rawSkillID] & 0x8000)) || useSubSkill)
        {
            ...
            // int16  Diff          = MaxSkill - CurSkill / 10;
            // MaxSkill - CurSkill / 10 doesn't make sense per the order of operations. This results in a larger chance to skill up the larger your skill number is. The exact opposite is true in live.
            // I suspect is should be (MaxSkill - CurSkill) / 10 which more accurately reflects being harder to skill up the closer to cap one is.
            double SkillUpChance = (Diff / 5.0 + (0.75 - log10(1.0 + CurSkill / 100))) * settings::get<double>("map.SKILLUP_CHANCE_MULTIPLIER");
            // double SkillUpChance = Diff / 5.0 + settings::get<double>("map.SKILLUP_CHANCE_MULTIPLIER") * (2.0 - log10(1.0 + CurSkill / 100));
            // Again the math doesn't make sense as presented. There are several things wrong with this equation. The resulting value will always be over 100% chance, even with my change on the previous equation, which makes it pointless as it will get set to .5
            // it also makes the skillup_chance_multiplier largely irrelevant. It has to be an absurd number to make a noticable difference currently. If it is going to be a true multiplier, it should be done on the whole equation.
            // also pretty sure the 2 - log should be 1 - log. this is largely responsible for the 100% skill up chance no matter what the values are. There is no way this is accurate for live.
            // The above equation represents what I think this math should be to more accurately reflect the live environment. No idea where they got the current math but they definitely didn't get it from live.
            // The range puts you anywhere from 75% skill up chance at the largest ranges or lowest levels to 5% at 423 trying for that last point to go to max of 424 on an A+ skill
            ...
            //    SkillUpChance = 0.5;
            //}
            // What is the point of having a multiplier if we are going to be capped at 50% anyway?
